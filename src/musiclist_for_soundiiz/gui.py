...
class MusicListGUI:
    ...
    def _create_input_section(self, parent):
        ...
        self.dir_listbox = tk.Listbox(
            list_frame, yscrollcommand=scrollbar.set, selectmode=tk.EXTENDED, height=2
        )
        ...
        self.remove_btn = ttk.Button(
            btn_frame, text="Remove Selected", command=self._remove_directory
        )
    ...
    def _create_output_section(self, parent):
        ...
        ttk.Entry(file_frame, textvariable=self.output_file).pack(
        ...
        for fmt in ["csv", "json", "m3u", "txt"]:
            ttk.Radiobutton(
                format_frame, text=fmt.upper(), variable=self.export_format, value=fmt
        ...
        ttk.Spinbox(
            max_songs_frame, from_=10, to=10000, textvariable=self.max_songs_per_file, width=10
        ).pack(side=tk.LEFT)
    ...
    def _create_options_section(self, parent):
        ...
        self.recursive_check = ttk.Checkbutton(
            self.options_frame, text="Scan subdirectories recursively", variable=self.recursive
        )
    ...
    def _create_progress_section(self, parent):
        ...
        self.progress_bar = ttk.Progressbar(
            self.progress_frame, mode="determinate", length=300, maximum=100
        )
    ...
    def _create_status_bar(self):
        ...
        self.status_bar = ttk.Label(
            self.root, text="Ready to process music files", relief=tk.SUNKEN, anchor=tk.W
        )
    ...
    def _browse_output(self):
        ...
        filename = filedialog.asksaveasfilename(
            title=_.get("output_file"), defaultextension=f".{ext}", filetypes=filetypes
        )
        if filename:
        ...
        try:
            ...
            for dir_idx, directory in enumerate(self.input_dirs, 1):
                ...
                metadata_list = extractor.extract_all(
                    directory=directory, recursive=self.recursive.get()
                )
                ...
                if duplicates:
                    ...
                    if self.remove_duplicates.get():
                        unique_list, removed_list = detector.remove_duplicates(
                            all_metadata, strategy=self.duplicate_strategy.get()
                        )
            ...
            messagebox.showinfo(
                _.get("success"),
                _.get(
                    "success_message", count=len(metadata_to_export), file=self.output_file.get()
                ),
            )
