...
def test_parse_args_duplicate_detection():
    ...
    args = parse_args(
        ["-i", "/path/to/music", "--detect-duplicates", "--duplicate-report", "report.txt"]
    )
...
def test_main_batch_processing(tmp_path):
    ...
    if test_file_src.exists():
        ...
        exit_code = main(["-i", str(dir1), str(dir2), "-o", str(output_file)])
...
def test_main_json_format(tmp_path):
    ...
    if test_file_src.exists():
        ...
        exit_code = main(["-i", str(music_dir), "-o", str(output_file), "-f", "json"])
